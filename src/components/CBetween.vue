...
<template>
  <div class="between-container">
    <CLeft class="absolute h-full w-58 bg-white z-10 top-0 left-0" v-if="isLeftToggleActive === true" />
    <header class="flex justify-between items-center text-gray-500">
      <i class="fas fa-bars text-2xl px-2 hidden" @click="onLeftToggleClick"></i>
      ...
      <div class="main-content__search-form flex justify-around px-5 py-2.25 rounded-2xl bg-white w-full">
        <i class="fas fa-search search-btn pr-3"></i>
        <input type="text" class="search-input outline-none border-none bg-transparent w-full"
    ...
    <div class="pt-4 pb-1">
      <p class="main-content__des text-[0.6rem]">
        What's hot<span><img src="/img/fire.png" alt="" width="16px" height="17px" /></span>
      </p>
    </div>
    ...
    <div class="my-playlist">
      <div class="playlist-head pt-5 flex justify-between items-center">
        <h4>Top 100 VN</h4>
        <a
          class="playlist-more btn-hover hover:(text-white bg-black px-2) rounded-3xl duration-200 cursor-pointer text-xs">Show
          all</a>
      </div>
      <div class="playlist-list flex justify-between text-xs pr-3 py-1 text-gray-600">
    ...
    <div class="overflow-y-scroll h-56 my-1">
      <div v-if="!isLoading">
        <div v-for="(index, i) in main.results.length" :key="i"
    ...
    <div class="play-song p-3 bg-white rounded-lg">
      <div class="play-control flex justify-between items-center">
        <div class="play-control__act flex justify-between items-center text-gray-500 w-[17%]">
          <i class="far fa-heart" @click="onHeart" v-show="isHeart === false"></i>
          <i class="fas fa-heart text-red-600" @click="onHeart" v-show="isHeart === true"></i>
          <i class="fas fa-music"></i>
          <i class="fas fa-expand-alt"></i>
        </div>
        <div class="play-control__main w-[28%] flex justify-between items-center cursor-pointer">
          <i class="fas fa-redo-alt play-repeat text-gray-500 text-sm"></i>
          <i class="fas fa-fast-backward play-backward main-icon"></i>
          <i class="fas fa-pause-circle text-3xl" @click="onPlaySong" v-show="isPlaying"></i>
          <i class="fas fa-play-circle text-3xl" @click="onPlaySong" v-show="!isPlaying"></i>
          <i class="fas fa-fast-forward play-forward main-icon"></i>
          <i class="fas fa-random shuffle-song text-gray-500 text-sm"></i>
        </div>
        <div class="play-control__volume w-[25%] flex justify-between items-center">
          <i class="fas fa-volume-down text-gray-500"></i>
          <div class="slidecontainer1 relative">
            <span class="w-[90%] h-0.25 bg-red-500 absolute top-1/2 left-1"></span>
            <input type="range" min="1" max="100" class="slider1" id="myRange1" v-model="currentVolume" />
          </div>
      ...
      <div class="play-seekbar flex justify-between items-center">
        ...
        <div class="slidecontainer2 relative">
          <span class="w-full h-0.75 bg-gray-500 absolute top-1/2 left-0"></span>
          <input type="range" min="1" max="100" class="slider2" id="myRange2" v-model="currentTime" />
        </div>
